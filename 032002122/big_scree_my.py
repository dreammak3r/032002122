...
def page_draggable_layout(today_covid_adata, today_not_app_covid_adata, line_time_data, line_covid_data, line_not_app_covid_data, bar_time_data, bar_covid_data, bar_not_app_covid_data):
...
def word_cloud_creat(adata, ndata, a):
...
#写入港澳台新增数据
def write_outland_into_excel(workbook, today_covid_adata, range_up_limit, whether_write, write_row):
    sheet_a=workbook["新增普通"]

    if whether_write == True:
        for i in range((range_up_limit - 1) * 24 - 1):
            for j in range(34, 37):
                sheet_a.cell(i + 2, j).value = sheet_a.cell(i + 2, j + 3).value - sheet_a.cell(i + 3, j + 3).value

    today_covid_adata["台湾新增"] = sheet_a.cell(write_row, 34).value
    today_covid_adata["香港新增"] = sheet_a.cell(write_row, 35).value
    today_covid_adata["澳门新增"] = sheet_a.cell(write_row, 36).value

    workbook.save("covid-19.xlsx")
    return today_covid_adata

#创间平面时间轴地图
def map_timeline_creat(adata, date, ndata):
    today_covid_data = [
        ("黑龙江", adata["黑龙江"]),
        ("内蒙古",adata["内蒙古"]),
        ("吉林", adata["吉林"]),
    ...
    map0 = (
        Map(init_opts=opts.InitOpts(width="1600px", height="900px", theme=ThemeType.WESTEROS, chart_id="map2d"))
        .add('新增普通确诊', data_pair=today_covid_data)
        .add('新增无症状确诊', data_pair=today_not_covid_data)
        .set_global_opts(
            title_opts=opts.TitleOpts(title=date),
...
def bar_create(bar_time_data, bar_covid_data, bar_not_app_covid_data):
    ...
    c = (
        Bar(init_opts=opts.InitOpts(width="1600px", height="900px", theme=ThemeType.WESTEROS, chart_id="bar"))
        ...
        .set_series_opts(itemstyle_opts={  # 颜色、样式设置
                    "normal": {
                        "barBorderRadius": [30]*4,
                        "shadowColor": "rgb(0,168,225)",
                    }
                })
    )
    return c
...
def line_create(line_time_data, line_covid_data, line_not_app_covid_data):
    ...
    c = (
        Line(init_opts=opts.InitOpts(width="1600px", height="900px", theme=ThemeType.WESTEROS, chart_id="line"))
        .set_global_opts(
        ...
        .add_yaxis(
            ...
            symbol="emptyCircle",
            is_symbol_show=True,
            label_opts=opts.LabelOpts(is_show=True),linestyle_opts=opts.LineStyleOpts(width=5)
        )
        .add_yaxis(
            series_name="新增无症状确诊",
...
def map_3d_creat(adata, ndata):
        ...
        #删除值为0的数据
        for x in today_covid_data:
            for n in today_covid_data:
                if n[1][2] == 0:
                    today_covid_data.remove(n)

        #删除值为0的数据
        for x in today_not_app_covid_data:
            for n in today_not_app_covid_data:
                if n[1][2] == 0:
                    today_not_app_covid_data.remove(n)


        map = (
            Map3D(init_opts=opts.InitOpts(width="1600px", height="900px",theme=ThemeType.WESTEROS, chart_id="map3d"))
            .add_schema(
                box_height = 50,
                region_height=1,
                view_control_opts=opts.Map3DViewControlOpts( pan_mouse_button='middle', rotate_mouse_button='left'),
...
#写入excel表格中
def write_into_xls(workbook, time_data_final, i, count_line, adata, ndata):
    #获取表格工作表
    sheet_a = workbook["新增普通"]
...
def crwal_and_text(range_up_limit, workbook):
    ...
    for i in range(1, range_up_limit):
        ...
        for html_ite, title_c, count_line in zip(html, title, range(len(html))):
            ...
            temp_1 = ''.join(says_1)
            says_2 = re.findall(second_pattern, temp_1)
            temp_2 = '\n'.join(says_2)
            final_says = re.findall(yiqing_pattern, temp_2)#每个省份的疫情信息存入列表
            ...
            write_into_xls(workbook, time_data_final, i, count_line,  adata, ndata)
            ...
            if (count_line == 0 and i == 1):
                ...
                today_date = time_data_final

            #平面地图所需数据
            if(i == 1 and count_line<7):
                adata_list.append(adata)
                ndata_list.append(ndata)
                time_data_final_list.append(time_data_final)




            if (count_line == len(html) - 1 and i == range_up_limit-1):
                #3D地图所需数据=====================================================================================
                today_covid_adata = write_outland_into_excel(workbook, today_covid_adata, range_up_limit, True, 2)
                ...
                adata_list.reverse()
                ndata_list.reverse()
                time_data_final_list.reverse()
                for x in range(7):
                    adata_list[x] = write_outland_into_excel(workbook, adata_list[x], range_up_limit, False, 8-x)
                    tl.add(map_timeline_creat(adata_list[x], time_data_final_list[x], ndata_list[x]), time_data_final_list[x])
                #==============================================================================================================
                #创建所有图表
                page_draggable_layout(today_covid_adata, today_not_app_covid_adata, line_time_data, line_covid_data, line_not_app_covid_data, bar_time_data,
