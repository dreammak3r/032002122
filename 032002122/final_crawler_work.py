...
#写入港澳台新增数据
def write_outland_into_excel(workbook, today_covid_adata, range_up_limit):
...
def map_timeline_creat(adata, date, ndata):
    ...
    map0 = (
        Map(init_opts=opts.InitOpts(width="1600px", height="900px", theme="essos"))
...
def map_3d_creat(adata, ndata):
        ...
        map = (
            Map3D(init_opts=opts.InitOpts(width="1600px", height="900px",theme="essos"))
            .add_schema(
                box_height = 50,
                region_height=1,
    ...
    for i in range(1, range_up_limit):
        ...
        for html_ite, title_c, count_line in zip(html, title, range(len(html))):
            ...
            if(i == 1 and count_line<7):
                tl.add(map_timeline_creat(adata, time_data_final, ndata), time_data_final)



            if (count_line == len(html) - 1 and i == range_up_limit-1):
                today_covid_adata = write_outland_into_excel(workbook, today_covid_adata, range_up_limit)
                creat_all_charts(today_covid_adata, today_not_app_covid_adata, line_time_data, line_covid_data, line_not_app_covid_data, bar_time_data,
                                 bar_covid_data, bar_not_app_covid_data, tl)
...
#创建所有图表
def creat_all_charts(today_covid_adata, today_not_app_covid_adata, line_time_data, line_covid_data, line_not_app_covid_data, bar_time_data, bar_covid_data, bar_not_app_covid_data, tl):
    #分页创建所有图表
    tab = Tab()
    tab.add(map_3d_creat(today_covid_adata, today_not_app_covid_adata), "今日确诊地图")
    tab.add(line_create(line_time_data, line_covid_data, line_not_app_covid_data), "本土确诊折线图")
    tab.add(bar_create(bar_time_data, bar_covid_data, bar_not_app_covid_data), "本土确诊柱状图")
    tab.add(tl, "七日疫情地图")
