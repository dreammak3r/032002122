...
export class BattleControl extends Component {
    ...
    start() {
        this.AudioCompment = this.node.getComponent('cc.AudioSource');
    }
    initial() {
        //生成A玩家骰子显示
        ...
        bPlayer.setPosition(95.047, -27.274);
        //获取父节点
        ...
        this.node.on('roundChange',function(event){
            ...
            if(this.boradFullCheck())
            {
                ...
                {
                    //A玩家胜利
                    this.showWinBorad(aScore,bScore,1385.544,1020,"WinA");
                    return;
                }
                else if(aScore < bScore)
                {
                    //B玩家胜利
                    this.showWinBorad(aScore,bScore,1385.544,1020,"WinB");
                    return;
                }
                else
                {
                    this.showWinBorad(aScore,bScore,1385.544,1020,"NotWin");
                    return;
                    //平局
                }
    ...
    clean(){
        ...
        let DoubleOver = this.myParent.getChildByName('DoubleOver');
        let AScore = DoubleOver.getChildByName('AScore');
        let BScore = DoubleOver.getChildByName('BScore');
        if(DoubleOver.getChildByName('WinWordA'))
        {
            WinWordA = DoubleOver.getChildByName('WinWordA');
            WinWordA.destroy();
        }
        if(DoubleOver.getChildByName('WinWordB'))
        {
            WinWordB = DoubleOver.getChildByName('WinWordB');
            WinWordB.destroy();
        }
        if(DoubleOver.getChildByName('NotWinWord'))
        {
            NotWinWord = DoubleOver.getChildByName('NotWinWord');
            NotWinWord.destroy();
        }
        if(aPlayerNum!=null)
            aPlayerNum.destroy();
    ...
    showWinBorad(aScore,bScore,x,y,result){
        let GameOver:Node = this.myParent.getChildByName('DoubleOver');
        let aScoreNode = instantiate(this.aScorepre);
        let bScoreNode = instantiate(this.bScorepre);
        aScoreNode.setParent(GameOver);
        aScoreNode.setPosition(0,150.582);
        bScoreNode.setParent(GameOver);
        ...
        if(result == "WinA")
        {
            this.putWinWord(this.WinWordApre,GameOver);
        }
        else if(result == "WinB")
        {
            this.putWinWord(this.WinWordBpre,GameOver);
        }
        else
        {
            this.putWinWord(this.NotWinpre,GameOver);
        }
    }
